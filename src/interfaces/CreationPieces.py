...
class CreationPieces(tk.Frame):
    ...
    def __init__(self, controller, num_defi, counter_values):
        ...
        tk.Button(self.grid_container, text="Ajouter", font=("Arial", 14, "bold"), bg="blue", fg="white", command=self.valider_piece, width=20).grid(row=2, column=0, pady=10)
        tk.Button(self.grid_container, text="Effacer dernière pièce", font=("Arial", 14, "bold"), bg="red", fg="white", command=self.effacer_derniere_piece, width=20).grid(row=3, column=0, pady=10)
        tk.Button(self.grid_container, text="Retour", font=("Arial", 14, "bold"), bg="darkred", fg="white", command=self.retour_option_generation, width=20).grid(row=1, column=0, pady=10)
        tk.Button(self.grid_container, text="Valider", font=("Arial", 14, "bold"), bg="blue", fg="white", command=self.valider_piece, width=20).grid(row=2, column=0, pady=10)

        # Conteneur pour l'aperçu des pièces créées (une seule ligne d'aperçu)
        self.aperçu_frame = tk.Frame(self, bg="#004A9A")
        self.aperçu_frame.grid(row=1, column=1, padx=10, pady=10, sticky="w")
    ...
    def effacer_derniere_piece(self):
        if self.pieces:
            ...
            for widget in self.aperçu_frame.winfo_children():
    ...
    def aperçu_piece(self, nouvelle_piece):
        ...
        frame = tk.Frame(self.aperçu_frame, bg="#004A9A")
    ...
    def retour_option_generation(self):
        from src.interfaces.OptionsGeneration import OptionsGeneration  # Import différé pour éviter la boucle d'import
